...
def test_query_duplicate_in():
    """Test query with --duplicate-in"""

    runner = CliRunner()
    cwd = os.getcwd()
    result = runner.invoke(
        query,
        ["--json", "--db", os.path.join(cwd, PHOTOS_DB_15_7),
         "--duplicate-in", os.path.join(cwd, PHOTOS_DB_14_6)],
    )
    assert result.exit_code == 0
    ...
    json_got = json.loads(result.output)
    uuid_got = [photo["uuid"] for photo in json_got]
    assert sorted(uuid_got) == sorted(UUID_DUPLICATES_IN)

def test_query_no_duplicate_in():
    """Test query with --no-duplicate-in"""
    ...
    result = runner.invoke(
        query,
        ["--json", "--db", os.path.join(cwd, PHOTOS_DB_15_7),
         "--no-duplicate-in", os.path.join(cwd, PHOTOS_DB_14_6)],
    )
    assert result.exit_code == 0
    ...
    json_got = json.loads(result.output)
    uuid_got = [photo["uuid"] for photo in json_got]
    assert sorted(uuid_got) == sorted(UUID_NO_DUPLICATES_IN)
